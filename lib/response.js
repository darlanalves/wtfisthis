...
Response.create = function(error, response) {
	var isError = Boolean(error || !response || (200 < response.statusCode || response.statusCode >= 400));

	var headers = response ? response.headers : {};

	var contentType = headers['content-type'] || '';
	var contentSize = headers['content-length'] || (response && response._body || '').length;

	var code = isError ? (response ? response.statusCode : 400) : (response.statusCode || 200);
	var errorMessage = isError ? String(error || response._body || '').slice(0, 256) : false;
